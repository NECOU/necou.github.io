...
We also discovered an optimal balance between the two models: a weight ratio favoring the VLM (60%) while incorporating moderate LLM influence (40%) delivered the best performance. This suggests that while the LLM's language understanding is valuable, preserving the VLM's multimodal capabilities remains paramount for maintaining strong performance on visual-language tasks.

These findings open new avenues for developing more effective multimodal AI systems that thoughtfully combine the strengths of specialized models rather than relying on brute-force merging or costly retraining approaches.

## Introduction

Large Language Models (LLMs) and Vision-Language Models (VLMs) have seen remarkable advancements in recent years. While VLMs can process both visual and textual information, they may not match the sophisticated text understanding capabilities of specialized LLMs. This raises an interesting question: can we combine the strengths of both model types to create a more powerful multimodal system?

In this article, we explore a novel approach to model fusion, focusing on selectively merging parameters between a VLM and an LLM that share the same base architecture. Rather than merging models indiscriminately, we investigate which components benefit most from fusion and how different merging strategies affect performance across multimodal tasks.
